...
switch ($method) {
    ...
    case 'update_scss_settings':
        ...
        isset($data->enqueue_aktiv) && is_string($data->enqueue_aktiv) ? $minify_enqueue_aktiv = 1 : $minify_enqueue_aktiv = 0;
        isset($data->cache_aktiv) && is_string($data->cache_aktiv) ? $minify_cache_aktiv = 1 : $minify_cache_aktiv = 0;
        isset($data->cache_path) && is_string($data->cache_path) ? $minify_cache_path = esc_textarea($data->cache_path) : $minify_cache_path = '';

        if (!$minify_cache_path) {
            $minify_cache_path = WP_CONTENT_DIR . DIRECTORY_SEPARATOR . 'scss_cache';
        }

        if (!$minify_cache_aktiv) {
            $SCSS_compiler->delete_scss_compiler_cache($minify_cache_path);
        } else {
            $SCSS_compiler->check_if_dir(get_option('minify_cache_path'));
        }
        ...
        update_option('minify_enqueue_aktiv', $minify_enqueue_aktiv);
        update_option('minify_cache_aktiv', $minify_cache_aktiv);
        update_option('minify_cache_path', $minify_cache_path);
        $responseJson->status = true;
        $responseJson->msg = date('H:i:s', current_time('timestamp'));
        break;
    case'minify_clear_cache':
        $dir = get_option('minify_cache_path');
        $SCSS_compiler->delete_scss_compiler_cache($dir);
        $responseJson->status = true;
