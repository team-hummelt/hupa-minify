...
function send_xhr_license_data(data) {
    ...
    xhr.open('POST', hupa_minify_license_obj.ajax_url, true);
    let input = new FormData(data);
    for (let [name, value] of input) formData.append(name, value);

    formData.append('_ajax_nonce', hupa_minify_license_obj.nonce);
    formData.append('action', 'HupaMinifyLicenceHandle');
    xhr.send(formData);
    //Response
    xhr.onload = function () {
